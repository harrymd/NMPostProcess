...
def get_indices_of_regions(nodes, node_attbs, node_idxs, r_discons, state_outer, i_outer_node, ellipticity_profile = None):
    '''
    ...
    # Case 2: There are some interior discontinuities.
        ...
        for i_shell in range(1, n_discons):
            ...
            _, j_discon = get_samples_on_boundary(i_shell, is_outer, nodes, node_attbs, node_idxs, r_discons, state_outer)
...
def new_get_samples_on_boundary(i_shell, r_discons, r_nodes, i_bd_node, node_idxs):
    
    #print('\n')
    #n_discons = len(r_discons)
    #r_discon_mid_points = r_discons[0:-1] + 0.5*np.diff(r_discons)

    #if i_shell == 0:

    #    cond_r = (r_nodes > r_discon_mid_points[0]) 

    #elif i_shell == (n_discons - 1):

    #    cond_r = (r_nodes < r_discon_mid_points[-1])

    #else:

    #    cond_r = (r_nodes > r_discon_mid_points[i_shell]) & (r_nodes < r_discon_mid_points[i_shell - 1])

    #cond_bd = (node_bd_flag == 1)

    #cond_node = (cond_r & cond_bd)

    #n_nodes = len(r_nodes)
    #i_node = np.array(range(n_nodes), dtype = np.int)

    #i_bd_node = i_node[cond_node]

    #cond_sample = np.array([(i in i_bd_node) for i in node_idxs], dtype = np.bool)
    #i_sample = np.where(cond_sample)[0]

    import matplotlib.pyplot as plt
    fig = plt.figure()
    ax = plt.gca()
    
    i_bd_node = np.array(list(i_bd_node), dtype = np.int)
    i_bd_node = np.sort(i_bd_node)
    print(i_bd_node)
    ax.hist(r_nodes[i_bd_node])
    ...
    #print(node_idxs)
    #print(len(node_idxs))
    #print(len(np.unique(node_idxs)))
    print(np.sum(cond_bd))
    
    #print(np.sum(cond_bd))
    #print(np.sum(cond_sample))
   #print(np.sum(cond_r))
    
    #print(np.sum(cond_node))
    #print(len(i_sample))
...
def get_samples_on_boundary(i_shell, is_outer, nodes, node_attbs, node_idxs, r_discons, state_outer, i_outer_node = None):
    '''
    ...
    if (i_shell == 0) and is_outer:
        ...
        assert i_outer_node is not None
        condition = np.array([(i in i_outer_node) for i in node_idxs], dtype = np.bool)
...
def pre_process(dir_PM, dir_NM, ellipticity_profile = None):
    ...
    nodes, node_bd_flag, _, _, _, _, face_list, face_attb, face_tets = read_mesh(dir_PM)
        ...
        for i in range(n_faces):
            ...
            if (face_tets[i, 0] == -2) | (face_tets[i, 1] == -2):
                ...
                for j in range(3):
                    ...
                    i_outer_node.add(face_list[i, j])
...
def read_mesh(dir_PM):
    ...
    node_info       = np.loadtxt(
                    ...
                    dtype       = np.float)
    nodes = node_info[:, 0:3]
    node_bd_flag = node_info[:, -1].astype(np.int)
    ...
    # Read the .face file.
    path_face_regex = os.path.join(dir_PM, '*.face')
    path_face = glob(path_face_regex)[0]
    face_info = np.loadtxt(
                    path_face,
                    comments = '#',
                    skiprows = 1,
                    usecols = (1, 2, 3, 4, 5, 6),
                    dtype = np.int)
    face_list = face_info[:, 0 : 3]
    face_attb = face_info[:, 3]
    face_tets = face_info[:, 4:]

    # Switch to 0-based indexing.
    face_list = face_list - 1
    face_tets = face_tets - 1
