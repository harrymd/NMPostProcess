...
from NMPostProcess.common import (get_list_of_modes_from_output_files,
        load_sample_indices_and_attribs,
...
def get_samples_on_boundary(i_shell, is_outer, nodes, node_attbs, node_idxs, r_discons, state_outer, i_outer_node = None):
    ...
    if (i_shell == 0) and is_outer:
        ...
        condition = np.array([np.isin(i, i_outer_node) for i in node_idxs], dtype = np.bool)
...
def pre_process(dir_PM, dir_NM, ellipticity_profile = None):
    ...
    if not os.path.exists(path_index_lists):
        ...
        for i in range(n_faces):
            ...
            if (face_tets[i, 0] == -2) | (face_tets[i, 1] == -2) | (face_tets[i, 0] == -1) | (face_tets[i, 1] == -1):
                
                for j in range(3):

                    i_outer_node.add(face_list[i, j])
        i_outer_node = np.array(list(i_outer_node), dtype = np.int)
...
def read_mesh_legacy(dir_PM):
    ...
    print(path_ele_regex)
    assert len(path_ele_regex_glob) == 1, 'Found 0 or more than one .ele file (excluding .b.ele).'
    ...
    assert len(path_node_regex_glob) == 1, 'Found 0 or more than one .node file (excluding .b.node).'
...
def find_r_max(nodes, node_idxs, eigvec, index_lists, real_or_complex, r_min = None, ellipticity_profile = None):
    ...
    return r_max, r_ell_max, i_region_max, S_max, j
...
def pre_projection(dir_NM, l_max, eigvec_path_base, nodes, node_idxs, node_attbs, index_lists, i_first_order, r_discons, i_mode, real_or_complex, ellipticity_profile = None):
    ...
    r_max, r_ell_max, i_region_max, eigvec_max, j_max = find_r_max(nodes, node_idxs, eigvec, index_lists, real_or_complex, r_min = 0.1*r_discons[0], ellipticity_profile = ellipticity_profile)
